...
algs = {
    'EGT_1E-3_TEST3': lambda args: egt.excessive_gap_technique_init(aggressive_stepsizes=args.aggressive_stepsizes,
                                                                    init_gap=init_gap, init_update_x=init_update_x,
                                                                    allowed_eps_increase=allowed_exp_increase,
                                                                    epsilon=0),
    'EGT_QPE_1E-2': lambda args: egt.excessive_gap_technique_init(aggressive_stepsizes=args.aggressive_stepsizes,
                                                                  init_gap=init_gap, init_update_x=init_update_x,
                                                                  allowed_exp_increase=allowed_exp_increase, qpe=True,
                                                                  epsilon_qpe=1e-2),

    # vanilla CFR, CFR+
    'CFR': lambda args: eqm_regret.regret_minimization_initializer(matrix_regret.regret_matching_initializer(),
                                                                   alternate=True, averaging='quadratic', name='CFR'),
    'CFR+_1E-3': lambda args: eqm_regret.regret_minimization_initializer(
    ...
    'RTCFR': lambda args: eqm_regret.regret_minimization_initializer(matrix_regret.regret_matching_initializer(),
                                                                     ...
                                                                     rt_step=50, rt_weight=0.0001, name='RTCFR'),

    'RTCFR+_0': lambda args: eqm_regret.regret_minimization_initializer(
        matrix_regret.regret_matching_plus_initializer(), alternate=True, is_last=True, rt_type=1, rt_step=1,
        rt_weight=0.001, is_tuned=False, rt_weight_tuned=[0.005, 0.001], rt_step_tuned=[20, 30, 40], iterate_num=100,
        epsilon=0, name='RTCFR+'),

    'RTCFR+_1E-2': lambda args: eqm_regret.regret_minimization_initializer(
        matrix_regret.regret_matching_plus_initializer(), alternate=True, is_last=True, rt_type=1, rt_step=5,
        rt_weight=0.01, is_tuned=False, rt_weight_tuned=[0.005, 0.001], rt_step_tuned=[20, 30, 40], iterate_num=100,
        epsilon=1e-2, name='RTCFR+'),
    'RTCFR+_ADP_1': lambda args: eqm_regret.regret_minimization_initializer(
        ...
        rt_step_tuned=[20, 30, 40], iterate_num=100, epsilon=1e-1, gamma=0.5, name='RTCFR+'),
    ...
    'OMWU_0': lambda args: md.mirror_descent_init(mirror_type=1, weight=None, is_last=True, is_tuned=False,
                                                  tuned_num=20, highest_multiplier=10, lowest_multiplier=0.01,
                                                  iterate_num=1000, epsilon=1e-15, name='OMWU'),

    'REGOMWU_1E-3': lambda args: md.mirror_descent_init(mirror_type=1, weight=0.0000335981828628378, is_last=True,
                                                        rt_type=3, rt_weight=1e-5, is_tuned=False, tuned_num=20,
                                                        highest_multiplier=1, lowest_multiplier=0.00001,
                                                        rt_weight_tuned=[1e-5], iterate_num=100, epsilon=1e-3,
                                                        name='Reg-OMWU'),

    'REGCFR_1E-3': lambda args: md.mirror_descent_init(mirror_type=2, weight=1, is_last=True, rt_type=3, rt_weight=1e-5,
                                                       is_tuned=False, tuned_num=20, highest_multiplier=1,
                                                       lowest_multiplier=1e-5, rt_weight_tuned=[1e-5], iterate_num=1000,
                                                       epsilon=1e-3, name='Reg-CFR'),

    'REGOMWU_EFPE_ADP': lambda args: md.mirror_descent_init(mirror_type=3, weight=None, is_last=True, rt_type=0,
                                                            ...
                                                            T=6000, name='REGOMWU_EFPE'),

    'REGDOMWU': lambda args: md.mirror_descent_init(predictive=True, weight=None, is_last=True, rt_type=3,
                                                    rt_weight=1e-7, is_tuned=True, tuned_num=20, highest_multiplier=10,
                                                    lowest_multiplier=0.01, rt_weight_tuned=[0.01], iterate_num=100,
                                                    name='Reg-DOMWU'),
...
parser.add_argument('-r', '--num_ranks', type=int, default=3, help='Number of ranks in the deck. Only works for Leduc.')
...
parser.add_argument('-a', '--algorithm', default=','.join(algs.keys()), dest='alg',
                    help='Available algorithms: %s' % ', '.join(
                        algs.keys()) + '. (a comma-separated list chooses several algorithms, ' + 'e.g. \'EGT,RM+\')')
...
file_path = "./plot_EFPE/%s/%s_%s_%s.txt" % (args.game, args.game, args.num_ranks, args.alg)
    ...
    for i in range(len(print_seq)):
        ...
        if to_csv:
            ...
            if False and str(opt) == 'ExcessiveGapTechnique':
                ...
                print('{iters}\t{grads}\t{exp}\t{profile_val}\t{time}\t{max_info_set_regret}'.format(
                    iters=print_seq[i], grads=opt.gradient_computations(), exp=exp, profile_val=profile_val,
                    time=total_time, max_info_set_regret=max_info_set_regret))
        else:
            print(print_seq[i], opt.gradient_computations(), exp, profile_val)
        print(print_seq[i], opt.gradient_computations(), exp, total_time, max_info_set_regret,
              file=gnuplot_out)
