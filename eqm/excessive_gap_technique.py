...
class ExcessiveGapTechnique(EquilibriumAlgorithm):
    def __init__(self, game, prox_x=None, prox_y=None, L=1.0, mu=2.0, aggressive_stepsizes=True, x=None, y=None,
        ...
        self._prox_x.set_epsilon(epsilon)
        self._prox_y.set_epsilon(epsilon)
    ...
    def initial_step_search(self):
        num_mu = 100
        # self._gradient_computations += 2 * int(np.ceil(np.log2(num_mu))) * 2 # remove it, for best initial
        ...
        for mu in np.logspace(np.log10(start), np.log10(1.0), num=num_mu):
            for lipschitz in [0.75, 1.0, 1.25]:
                ...
                if not self._init_update_x:
                    self._x = self._prox_x.center()  # L = 2.0*L
    ...
    def initial_step(self):
        try:
            ...
            _, _, self._y = self._smooth_br_y(-1.0, u_y, self._mu[1])

            u_x = self._game.utility_for(0, self._y)
            _, _, self._x = self._prox_x(-1.0, u_x, self._mu[0], self._x)
    ...
    def iterate(self, num_iterations=1):
        for _ in range(num_iterations):
            ...
            if self._aggressive_stepsizes:
                ...
                while self.excessive_gap() < 0 or self.worse_than_old(old_x, old_y, self._x, self._y):
                    self._gradient_computations += 2
                    self._mu = np.copy(old_mu)
                    self._tau *= 0.5
                    logging.getLogger().debug('%s, decreasing stepsize', self.get_params_string())
                    self.shrink(self._tau, old_x, old_y)
                if self._tau < 0.5:
                    self._tau *= 1.11
            else:
    ...
    def shrink(self, tau, x, y):
        ...
        if self._mu[0] > self._mu[1]:  # shrink mu for the player has bigger mu
            self._shrink_player(tau, x, y, 0, self._prox_x, self._smooth_br_x, self._smooth_br_y)
        else:
            self._shrink_player(tau, y, x, 1, self._prox_y, self._smooth_br_y, self._smooth_br_x)

    def _shrink_player(self, tau, x, y, player, prox_x, smooth_br_x, smooth_br_y):
        opponent = 1 - player
    ...
    def duality_gap_bound(self):
        return self._mu[0] * self._game.domain(0).diameter() + self._mu[1] * self._game.domain(1).diameter()
    ...
    def fit_to_strategy(self):
        if self.excessive_gap() < 0:
            while self.excessive_gap() < 0:
                self._mu *= 2
                gap = self.excessive_gap()
                logging.info('mu=[%0.2f, %0.2f] gap=%0.04f', self._mu[0], self._mu[1], gap)
        else:
            while self.excessive_gap() > 0:
                self._mu /= 2
                gap = self.excessive_gap()
...
def excessive_gap_technique_init(aggressive_stepsizes=False, **kwargs):
    def init(game):
        return ExcessiveGapTechnique(game, aggressive_stepsizes=aggressive_stepsizes, **kwargs)
...
def egt_warm_start_initializer(alg, aggressive_stepsizes=False, **kwargs):
    def init(game):
        ...
        opt.iterate(200)
        x, y = opt.profile()
        return ExcessiveGapTechnique(game, aggressive_stepsizes=aggressive_stepsizes, x=x, y=y, **kwargs)
