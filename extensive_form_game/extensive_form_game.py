...
class ExtensiveFormGame:
    ...
    def __init__(self, name, A, first, end, parent, seq_to_str=None, prox_infoset_weights=False, prox_scalar=1,
                 reach=None, all_negative=False, offset=0, B=None):
    ...
    def profile_exploitability(self, x, y, regularizer_x=0, regularizer_y=0):
        value_x, value_y = self.profile_value(x, y, regularizer_x, regularizer_y)
        ...
        if self._B is not None:
            ...
            return br_x + br_y, br_x - value_x, br_y - value_y, value_x, value_y

    def profile_value(self, x, y, regularizer_x=0, regularizer_y=0):
        seq1 = self.domain(0).sequence_form(x)
        seq2 = self.domain(1).sequence_form(y)
        return np.dot(seq1, self.utility_for(0, y, regularizer_x)), np.dot(seq2, self.utility_for(1, x, regularizer_y))

    def max_infoset_regret(self, x, y, epsilon):
        if epsilon != 0:
            return max(self.max_player_infoset_regret(0, x, y)[0], self.max_player_infoset_regret(1, y, x)[0])
        else:
            return -1
    ...
    def utility_for(self, player, opponent_strategy, regularizer=0):
        ...
        if player == 0:
            return -self._A.dot(seq) - regularizer

        assert player == 1
        if self._B is not None:
            return -self._B.dot(seq)
        return self._A_T.dot(seq) - regularizer
    ...
    def reach(self, player, opponent_strategy):
        ...
        if self._reach is None:
            raise ValueError("The reach argument in __init__ has to be set in order to call reach()")
        seq = self.domain(1 - player).sequence_form(opponent_strategy)
        return self._reach[player].dot(seq)

    def print_payoff_matrix(self, f=sys.stdout, negate=False, all_negative=False):
        constant_term = 0
        if all_negative:
    ...
    def __str__(self):
        return 'ExtensiveFormGame(%s, %dx%d)' % (self._name, self._A.shape[0], self._A.shape[1])
